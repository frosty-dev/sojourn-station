...
/obj/mecha/combat/gygax/verb/overload()
	...
	if(overload)
		...
		src.occupant_message("<span class='warning'>You enable leg actuators overload.</font>")
...
/obj/mecha/combat/gygax/do_move(direction)
	...
	if(overload)
		...
		if(health < initial(health) - initial(health)/3)
			...
			step_in = initial(step_in)
			step_energy_drain = initial(step_energy_drain)
			src.occupant_message("<span class='warning'>Leg actuators damage threshold exceded. Disabling overload.</font>")
	return
