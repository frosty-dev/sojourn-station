...
/obj/machinery/computer/message_monitor/emag_act(var/remaining_charges, var/mob/user)
	...
	if(!emag && operable())
		if(!isnull(src.linkedServer))
			...
			MK.info += "<br><br><span class='warning'>£%@%(*$%&(£&?*(%&£/{}</font>"
...
/obj/machinery/computer/message_monitor/attack_hand(var/mob/living/user as mob)
	...
	if(auth)
		dat += "<h4><dd><A href='?src=\ref[src];auth=1'>&#09;<span class='green'>\[Authenticated\]</span></a>&#09;/"
		dat += " Server Power: <A href='?src=\ref[src];active=1'>[src.linkedServer && src.linkedServer.active ? "<span class='green'>\[On\]</font>":"<span class='warning'>\[Off\]</span>"]</a></h4>"
	else
		dat += "<h4><dd><A href='?src=\ref[src];auth=1'>&#09;<span class='warning'>\[Unauthenticated\]</font></a>&#09;/"
		dat += " Server Power: <u>[src.linkedServer && src.linkedServer.active ? "<span class='green'>\[On\]</font>":"<span class='warning'>\[Off\]</span>"]</u></h4>"
	...
	switch(screen)
		...
		if(0)
			...
			if((isAI(user) || isrobot(user)) && (user.mind.antagonist.len && user.mind.original == user))
				...
				dat += "<dd><A href='?src=\ref[src];hack=1'><i><span class='warning'>*&@#. Bruteforce Key</font></i></font></a><br></dd>"
			else
				dat += "<br>"
