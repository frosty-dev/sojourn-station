...
/obj/machinery/mineral/processing_unit_console/interact(mob/user)
	...
	for(var/ore in machine.ores_processing)
		...
		if(machine.ores_processing[ore])
			switch(machine.ores_processing[ore])
				if(0)
					dat += "<span class='warning'>not processing</font>"
				if(1)
	...
	dat += "The ore processor is currently <A href='?src=\ref[src];toggle_power=1'>[(machine.active ? "<span class='green'>processing</font>" : "<span class='warning'>disabled</span>")]</a>."
